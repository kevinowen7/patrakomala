...
class ProductServices {
  ...
  static Future<ApiReturnValue<List<Product>>> filterProduct(int page,String produkName, List<String> subsector,
      {http.Client client}) async {
    client ??= http.Client();

    String url = baseURL2 + 'mobile/produk?page=$page&product_name=$produkName&subsektor=$subsector';
    var response = await client.get(
      url,
      headers: {"Content-Type": "application/json"},
    );
  ...
  static Future<ApiReturnValue<Bisnis>> getBisnis(int produkId,{http.Client client}) async {
    client ??= http.Client();
